...
function fillDayCompetitions(){
  ...
  const leagues=[...new Map(fixtures.filter(game=>!cpIsExcludedGame(game)&&cpLeagueMatchesMode(game.league,mode)&&game.league.name?.trim()).map(game=>[game.league.name.trim(),game.league])).values()].sort((a,b)=>cpLeagueDisplayName(a).localeCompare(cpLeagueDisplayName(b),'pt-BR'));
  const names=leagues.map(league=>league.name.trim());
  select.innerHTML='<option value="all">Todas as competições</option>'+leagues.map(league=>`<option value="${cpEscape(league.name.trim())}">${cpEscape(cpLeagueDisplayName(league))}</option>`).join('');
