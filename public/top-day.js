(function(){
  ...
  function gameCard(item,index){
    ...
    return`<article class="top-card rank-${rank}"><div class="rank-badge"><span>${medal(rank)}</span></div><div class="top-main"><div class="top-meta"><span>${cpEscape(cpLeagueDisplayName(game.league))}</span><span>${cpEscape(time(game.fixture.date))}</span></div><div class="top-teams"><div class="top-team home"><strong>${cpEscape(game.teams.home.name)}</strong>${teamLogo(game.teams.home)}</div><i>×</i><div class="top-team away">${teamLogo(game.teams.away)}<strong>${cpEscape(game.teams.away.name)}</strong></div></div><div class="market-title"><strong>${cpEscape(item.market.label)}</strong><span>${cpEscape(item.market.detail)}</span></div>${window.CPPrematchOdds?.placeholder(item)||''}<div class="sample-badges">${windowBadge(item,'l5','L5')}${windowBadge(item,'l10','L10')}<span>Amostra <b>${evidence.total}</b></span></div></div><div class="top-score"><span class="history-rate">${Math.round(item.metric.value)}%</span><span class="history-label">frequência histórica</span><span class="confidence-high">CONFIANÇA ALTA</span><div class="top-actions"><a href="match.html?id=${encodeURIComponent(game.fixture.id)}">Ver análise</a><a class="bankroll-link" data-odds-entry="${game.fixture.id}" data-odds-key="${item.key}" href="${bankrollHref(item)}">+ Minha Banca</a></div></div></article>`;
  }
  ...
  window.CPPrematchOdds?.mount($('topGamesList'));
  if(window.CPPrematchOdds)$('topGamesList').addEventListener('click',event=>{const link=event.target.closest('[data-odds-entry]');if(!link)return;const url=new URL(link.href),quote=window.CPPrematchOdds.selectedQuote(link.dataset.oddsEntry,link.dataset.oddsKey);url.searchParams.delete('odd');if(quote)url.searchParams.set('odd',quote.odd);link.href=url.href});
}());
