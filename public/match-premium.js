(function () {
  ...
  function enhanceBase(d) {
    ...
    const matchDate = date.toLocaleDateString('en-CA', { timeZone: 'America/Sao_Paulo' });
    const leagueGamesUrl = `games.html?date=${encodeURIComponent(matchDate)}&competition=${encodeURIComponent(fixture.league.id)}`;
    document.querySelector('.top')?.insertAdjacentHTML('afterend', `<nav class="match-breadcrumb" aria-label="Navegação estrutural"><a href="games.html">Partidas</a><i>›</i><a href="${leagueGamesUrl}">${safe(cpLeagueDisplayName(fixture.league))}</a><i>›</i><a class="current" href="match.html?id=${encodeURIComponent(fixture.fixture.id)}" aria-current="page">${safe(fixture.teams.home.name)} x ${safe(fixture.teams.away.name)}</a></nav>`);
