...
function dashboardHighlightCard(game,trend,analysis){const live=cpIsLive(game),time=live?cpStatus(game):cpTime(game);let trendMarkup;if(trend){trendMarkup=`<div class="featured-trend"><span><small>Melhor tendência</small><b>${trend.market.label}</b></span><strong class="trend-score"><span class="trend-percent">${Math.round(trend.metric.value)}%</span><small class="trend-main-sample">${trend.metric.hits}/${trend.metric.total}</small>${dashboardWindowBadges(analysis,trend.key)}</strong></div><span class="trend-progress"><i style="width:${Math.max(0,Math.min(100,Math.round(trend.metric.value)))}%"></i></span>`}else if(analysis?.analysisStatus==='insufficient'){trendMarkup='<div class="featured-preparing"><span>○</span><div><b>Dados insuficientes</b><small>A partida foi analisada, mas não há amostra válida suficiente para recomendar.</small></div></div>'}else if(analysis){trendMarkup='<div class="featured-preparing"><span>✓</span><div><b>Análise concluída</b><small>Sem destaque estatístico forte nesta partida no momento.</small></div></div>'}else{trendMarkup='<div class="featured-preparing"><span>⌁</span><div><b>Análise sendo preparada</b><small>Os dados aparecerão assim que estiverem disponíveis.</small></div></div>'}return `<article class="featured-match-card" data-match="${game.fixture.id}"><div class="featured-meta"><span>${cpEscape(game.league.name)}</span><em class="${live?'is-live':''}">${cpEscape(time)}</em></div><div class="featured-teams"><div>${dashboardTeamLogo(game.teams.home)}<strong>${cpEscape(game.teams.home.name)}</strong></div><span>×</span><div>${dashboardTeamLogo(game.teams.away)}<strong>${cpEscape(game.teams.away.name)}</strong></div></div>${trendMarkup}<a href="match.html?id=${game.fixture.id}">Ver partida <span>→</span></a></article>`}
