...
function dashboardHighlightCard(game,trend,analysis){const live=cpIsLive(game),time=live?cpStatus(game):cpTime(game);let trendMarkup;if(trend){trendMarkup=`<div class="featured-trend"><span><small>Melhor tendência</small><b>${trend.market.label}</b></span><strong>${Math.round(trend.metric.value)}% <small>${trend.metric.hits}/${trend.metric.total}</small></strong></div><span class="trend-progress"><i style="width:${Math.max(0,Math.min(100,Math.round(trend.metric.value)))}%"></i></span>`}else if(analysis?.analysisStatus==='insufficient'){trendMarkup='<div class="featured-preparing"><span>○</span><div><b>Dados insuficientes</b><small>A partida foi analisada, mas não há amostra válida suficiente para recomendar.</small></div></div>'}else if(analysis){trendMarkup='<div class="featured-preparing"><span>✓</span><div><b>Análise concluída</b><small>Nenhuma tendência atingiu os critérios mínimos neste momento.</small></div></div>'}else{trendMarkup='<div class="featured-preparing"><span>⌁</span><div><b>Análise sendo preparada</b><small>Os dados aparecerão assim que estiverem disponíveis.</small></div></div>'}return `<article class="featured-match-card" data-match="${game.fixture.id}"><div class="featured-meta"><span>${cpEscape(game.league.name)}</span><em class="${live?'is-live':''}">${cpEscape(time)}</em></div><div class="featured-teams"><div>${dashboardTeamLogo(game.teams.home)}<strong>${cpEscape(game.teams.home.name)}</strong></div><span>×</span><div>${dashboardTeamLogo(game.teams.away)}<strong>${cpEscape(game.teams.away.name)}</strong></div></div>${trendMarkup}<a href="match.html?id=${game.fixture.id}">Ver partida <span>→</span></a></article>`}

let dashboardHydrationDate=null;
async function hydrateDashboardAnalyses(date,games,snapshot){
  if(dashboardHydrationDate===date||typeof fixtureGoalMetrics!=="function")return;
  const existing=new Map((snapshot.games||[]).map(game=>[Number(game.fixture?.id),game]));
  const queue=games.filter(game=>!existing.has(Number(game.fixture.id)));
  if(!queue.length)return;
  dashboardHydrationDate=date;

  const publishProgress=()=>{
    const next={version:1,date,generatedAt:new Date().toISOString(),games:[...existing.values()]};
  ...
  const workers=Array.from({length:2},async()=>{
    while(queue.length){
      ...
      try{
        const params=new URLSearchParams({sample:'15',scope:'league',mode:'scanner',home:String(game.teams.home.id),away:String(game.teams.away.id),league:String(game.league.id),season:String(game.league.season)});
        ...
        if(result){
          existing.set(Number(game.fixture.id),{fixture:{id:game.fixture.id,date:game.fixture.date,status:game.fixture.status},league:game.league,teams:game.teams,metrics:result,cornerMetrics:null,analysisStatus:result.coverage>0?'ready':'insufficient'});
