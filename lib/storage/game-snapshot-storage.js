...
async function resolveGames(date, fetchGames, storage, options = {}) {
    const snapshotFreshMs = Math.max(0, Number(options.snapshotFreshMs) || 0);
    if (snapshotFreshMs > 0) {
        const snapshot = await storage.get(date);
        const generatedAt = Date.parse(snapshot?.generatedAt || "");
        if (snapshot && Number.isFinite(generatedAt) && Date.now() - generatedAt < snapshotFreshMs) {
            return { response: snapshot.fixtures, results: snapshot.count, source: "snapshot-cache", stale: false,
                snapshotGeneratedAt: snapshot.generatedAt, date: snapshot.date, offline: false };
        }
    }
    try {
