...
function notificationForTicket(ticket) {
  ...
  const odd = Number((ticket?.total_odd ?? ticket?.totalOdd) || 0).toFixed(2);
  const rawTitle = clean(ticket?.title, 100);
  const namedTicket = /(?:bingo|m[uú]ltipla)/i.test(rawTitle) ? rawTitle : '';
  const matches = [...new Set(picks.map(pick => clean(pick?.displayMatch, 120)).filter(Boolean))];
  let subject = namedTicket;
  if (!subject && matches.length === 1) subject = matches[0];
  if (!subject && matches.length > 1) subject = `${matches[0]} + ${matches.length - 1} jogo${matches.length - 1 === 1 ? '' : 's'}`;
  if (!subject) subject = 'Novo bilhete';
  const selectionCount = `${picks.length} ${picks.length === 1 ? 'seleção' : 'seleções'}`;
  const profile = clean(ticket?.analysis?.profile, 30);
  const firstFixture = picks.map(pick => pick?.fixtureDate).filter(Boolean).map(value => new Date(value)).filter(date => !Number.isNaN(date.getTime())).sort((a,b)=>a-b)[0];
  const firstGame = firstFixture ? new Intl.DateTimeFormat('pt-BR', { timeZone: 'America/Sao_Paulo', hour: '2-digit', minute: '2-digit' }).format(firstFixture) : '';
  const meta = [selectionCount, profile ? profile.charAt(0) + profile.slice(1).toLowerCase() : '', firstGame ? `1º jogo ${firstGame}` : ''].filter(Boolean).join(' · ');
  const body = `${subject} · Odd ${odd}\n${meta}`;
