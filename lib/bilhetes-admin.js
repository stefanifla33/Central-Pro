...
function buildManualTicket(input, now = new Date()) {
  ...
  const ticketUrl = clean(input.ticketUrl, 1000);
  if (ticketUrl && !/^https?:\/\//i.test(ticketUrl)) throw Object.assign(new Error('O link do bilhete precisa começar com http:// ou https://.'), { status: 400 });
  return { id, schema_version: 1, published_at: iso, date, type: 'MANUAL', title, description: clean(input.description, 180) || 'Bilhete selecionado pela Central Pro.', competition: clean(input.competition, 120) || 'Seleção manual', status: 'OPEN', total_odd: totalOdd, selections, analysis: { manual: true, ticketUrl: ticketUrl || null }, source: 'manual', settled_at: null };
}
