...
async function authenticateAdmin(authorization, { env = process.env, fetchImpl = global.fetch } = {}) {
  ...
  if (!url || !key || (!admin.email && !admin.userId) || typeof fetchImpl !== 'function') return null;
...
function normalizeSelections(input) {
  ...
  return input.map((item, index) => {
    ...
    if (!displayMatch || !market || !displaySelection || !Number.isFinite(odd) || odd < 1.01 || odd > 1000) throw Object.assign(new Error(`Seleção ${index + 1} incompleta ou com odd inválida.`), { status: 400 });
    return { id: `MANUAL-PICK-${index + 1}`, fixtureId: item.fixtureId ? Number(item.fixtureId) : null, fixtureDate: clean(item.fixtureDate, 40) || null, competitionName: clean(item.competitionName, 100) || null, market, marketName: market, marketKey: clean(item.marketKey, 80) || 'manual', displayMatch, displaySelection, odd: Number(odd.toFixed(2)), bookmakerName: bookmakerName || null, status: 'OPEN', metadata: { manual: true } };
  });
}
function buildManualTicket(input, now = new Date()) {
  ...
  const totalOdd = Number(selections.reduce((acc, item) => acc * item.odd, 1).toFixed(2));
  ...
  return { id, schema_version: 1, published_at: iso, date, type: 'MANUAL', title, description: clean(input.description, 180) || 'Bilhete selecionado pela Central Pro.', competition: clean(input.competition, 120) || 'Seleção manual', status: 'OPEN', total_odd: totalOdd, selections, analysis: { manual: true }, source: 'manual', settled_at: null };
}
