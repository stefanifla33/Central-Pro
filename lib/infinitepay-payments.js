...
const INFINITEPAY_PRODUCTION_BASE_URL = "https://central-pro.vercel.app";

function infinitePayCallbackBase({ nodeEnv = process.env.NODE_ENV, vercel = process.env.VERCEL, port = process.env.PORT || 3000 } = {}) {
    if (String(nodeEnv || "").toLowerCase() === "production" || String(vercel || "") === "1") {
        return INFINITEPAY_PRODUCTION_BASE_URL;
    }
...
function createInfinitePayPaymentService({
    ...
    async function verifyAndApply({ orderNsu, transactionNsu, slug, userId = null }) {
        ...
        if (userId && String(order.user_id) !== String(userId)) {
            return { httpStatus: 403, body: { success: false, message: "Pedido não pertence ao usuário" } };
        }
        ...
        logger.info("[INFINITEPAY-PAYMENT] payment verified", { orderNsu: order.id, duplicate: Boolean(result?.duplicate) });
    ...
    async function confirmPayment({ authorization, orderNsu, transactionNsu, slug }) {
        if (!infinitePay.configured() || !adminKey || !url) return { httpStatus: 503, body: { error: "payments_not_configured" } };
        const user = await validateUser(authorization);
        if (!user) return { httpStatus: 401, body: { error: "unauthorized" } };
        return verifyAndApply({ orderNsu, transactionNsu, slug, userId: user.id });
    }

    return { createCheckout, processWebhook, confirmPayment };
}

module.exports = { UUID_PATTERN, INFINITEPAY_PRODUCTION_BASE_URL, infinitePayCallbackBase, createInfinitePayPaymentService };
