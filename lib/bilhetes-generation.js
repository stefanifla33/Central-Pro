...
const HISTORY_TTL_MS = Math.max(900_000, Number(process.env.BILHETES_HISTORY_TTL_MS) || 21_600_000);
const STANDINGS_MARKETS = new Set(['homeWin', 'awayWin', 'homeOrDraw', 'awayOrDraw', 'homeWinEitherHalf', 'awayWinEitherHalf']);
...
const STAT_MARKETS = Object.freeze(['over05HT', 'over15', 'over25', 'btts', 'homeWin', 'awayWin', 'homeOrDraw', 'awayOrDraw', 'homeWinEitherHalf', 'awayWinEitherHalf', 'homeScores', 'awayScores']);
function statisticCandidates(games, marketKeys = STAT_MARKETS) {
  const activeMarkets = STAT_MARKETS.filter(key => (marketKeys || STAT_MARKETS).includes(key));
  const candidates = [], rejected = [];
  for (const game of games || []) {
    const metrics = game.metrics || game.sampleMetrics || {};
    const found = activeMarkets.filter(key => {
    ...
    for (const key of activeMarkets.filter(key => game.analyses?.[key] && !found.includes(key))) rejected.push({ fixtureId: game.fixture?.id || null, marketKey: key, reason: 'analysis_below_quality_threshold', analysis: game.analyses[key] });
...
async function generateBilhetesPreview({ games, date, oddsService, sharedCache, football, apiUsageDiagnostic, focusFixtureId = null, forceFocusFixture = false, marketKeys = STAT_MARKETS }) {
  const activeMarkets = STAT_MARKETS.filter(key => (marketKeys || STAT_MARKETS).includes(key));
  const focusId = Number(focusFixtureId) || null;
  const gamesOnDate = (games || []).filter(game => {
    const statusOk = ['NS','TBD'].includes(game.fixture?.status?.short);
    const dateOk = !date || new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Sao_Paulo' }).format(new Date(game.fixture.date)) === date;
    const timeOk = forceFocusFixture && focusId && Number(game.fixture?.id) === focusId
      ? true
      : Date.parse(game.fixture.date) >= Date.now() + (2 * 60 * 60 * 1000);
    return statusOk && dateOk && timeOk;
  });
  let eligibleGames = cpSelectScannerFixtures(gamesOnDate);
  if (forceFocusFixture && focusId) {
    const focus = gamesOnDate.find(game => Number(game.fixture?.id) === focusId);
    if (focus && !eligibleGames.some(game => Number(game.fixture?.id) === focusId)) eligibleGames = [focus, ...eligibleGames];
    eligibleGames = eligibleGames.filter(game => Number(game.fixture?.id) === focusId);
  }
  const standingsCache = new Map();
  const needsStandings = activeMarkets.some(key => STANDINGS_MARKETS.has(key));
  if (football && needsStandings) eligibleGames = await hydrateMatchContext(eligibleGames, { football, standingsCache });
  ...
  if (football && teams.length) {
    ...
    await mapWithConcurrency(teams, HISTORY_CONCURRENCY, async team => { try { const response = await football(`/fixtures?team=${team.id}&last=10&timezone=America%2FSao_Paulo`, HISTORY_TTL_MS); histories.set(team.id, (response.response || []).filter(item => ['FT','AET','PEN'].includes(item.fixture?.status?.short))); } catch { histories.set(team.id, []); } });
    for (const game of eligibleGames) {
      ...
      const goalEvidence = key => {
        if (key === 'over05HT') {
          const withHalftime = all.filter(item => Number.isFinite(Number(item.score?.halftime?.home)) && Number.isFinite(Number(item.score?.halftime?.away)));
          return pct(withHalftime, item => Number(item.score.halftime.home) + Number(item.score.halftime.away) >= 1);
        }
        return pct(all.filter(item => Number.isFinite(Number(item.goals?.home)) && Number.isFinite(Number(item.goals?.away))), item => {
      ...
      const metrics = {
        over05HT: { evidence: goalEvidence('over05HT') }, over15: { evidence: goalEvidence('over15') }, over25: { evidence: goalEvidence('over25') }, btts: { evidence: goalEvidence('btts') },
      ...
      const analyses = {};
      for (const marketKey of activeMarkets) analyses[marketKey] = analyzeMatch({ ...game, marketKey }, { [game.teams.home.id]: homeHistory, [game.teams.away.id]: awayHistory });
  ...
  const statistics = statisticCandidates(hydratedGames, activeMarkets);
  ...
  report.statisticalShortlist = qualifiedRequests.map(item => ({
    fixtureId: Number(item.game.fixture?.id),
    fixtureDate: item.game.fixture?.date || null,
    competitionId: Number(item.game.league?.id) || null,
    competition: item.game.league?.name || '',
    homeTeam: item.game.teams?.home || null,
    awayTeam: item.game.teams?.away || null,
    marketKey: item.marketKey,
    evidence: item.game.metrics || item.game.sampleMetrics || null,
    analysis: item.game.analyses?.[item.marketKey] || item.game.analysis || null
  }));
