...
function teamRows(history, teamId, venue) {
  return (history || []).map(item => {
    ...
    const htHome = Number(item.score?.halftime?.home), htAway = Number(item.score?.halftime?.away);
    const over05HT = finite(htHome) && finite(htAway) ? htHome + htAway >= 1 : null;
    return { gf, ga, won: gf > ga, draw: gf === ga, notLost: gf >= ga, scored: gf > 0, conceded: ga > 0, btts: gf > 0 && ga > 0, over05HT, over15: gf + ga >= 2, over25: gf + ga >= 3, clean: ga === 0 };
  }).filter(Boolean);
}
function form(rows) {
  ...
  const metric = key => {
    const usableRecent = recent.filter(r => r[key] !== null && r[key] !== undefined);
    const usableLast5 = last5.filter(r => r[key] !== null && r[key] !== undefined);
    return { ...pct(usableRecent, r => r[key]), recent5: pct(usableLast5, r => r[key]), weighted: weighted(r => r[key]) };
  };
  return { sampleSize: recent.length, last5: last5.length, wins: pct(recent, r => r.won), draws: pct(recent, r => r.draw), losses: pct(recent, r => !r.won && !r.draw), goalsFor: mean(recent.map(r => r.gf)), goalsAgainst: mean(recent.map(r => r.ga)), notLost: metric('notLost'), scored: metric('scored'), conceded: metric('conceded'), btts: metric('btts'), over05HT: metric('over05HT'), over15: metric('over15'), over25: metric('over25'), cleanSheets: metric('clean') };
}
...
function analyzeMatch(game, histories = {}) {
  ...
  else if (market === 'over05HT') { add('home.over05HT', val(venue.home, 'over05HT')); add('away.over05HT', val(venue.away, 'over05HT')); add('home.overall.over05HT', val(overall.home, 'over05HT')); add('away.overall.over05HT', val(overall.away, 'over05HT')); }
