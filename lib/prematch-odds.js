...
const MARKETS = Object.freeze({
    ...
    homeWin: [1, "Match Winner", "Home"],
    awayWin: [1, "Match Winner", "Away"],
    homeOrDraw: [12, "Double Chance", "Home/Draw"],
    awayOrDraw: [12, "Double Chance", "Draw/Away"],
    homeWinEitherHalf: [39, "To Win Either Half", "Home"],
    awayWinEitherHalf: [39, "To Win Either Half", "Away"],
...
function createPrematchOddsService({ football, now = Date.now, cacheExpiry = () => now() + TTL_MS, sharedCache = null }) {
    const cache = new Map(), pending = new Map();
    async function load(fixtureId, knownFixture = null) {
        const started = now();
        ...
        try {
            let rawExpiry = Date.now() + TTL_MS;
            const fixture = knownFixture || checked(await football(`/fixtures?id=${fixtureId}`, TTL_MS)).response.find(item => item.fixture?.id === fixtureId)?.fixture;
            ...
            const normalized = { fixtureId, status: quotes.length ? "available" : "unavailable", reason: quotes.length ? null : "no_exact_quotes", fixtureStatus: fixture.status.short, kickoff: fixture.date, fetchedAt: new Date(started).toISOString(), expiresAt: new Date(expires).toISOString(), selections };
            if (sharedCache?.setNormalizedOdds) await sharedCache.setNormalizedOdds(fixtureId, normalized, Math.max(0, expires - now()));
            return normalized;
        } catch {
            return unavailable("provider_unavailable");
        }
    }
    async function get(fixtureId, options = {}) {
        ...
        const request = load(fixtureId, options.fixture || null).then(result => {
    ...
    function getCachedOdds(fixtureId) {
        const entry = cache.get(Number(fixtureId));
        if (!entry || !entry.expiresAt || Date.parse(entry.expiresAt) <= now()) return null;
        return JSON.parse(JSON.stringify(entry));
    }
    return { get, getCachedOdds };
}
