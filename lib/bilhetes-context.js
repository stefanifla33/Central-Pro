...
async function hydrateMatchContext(games, { football, standingsCache = new Map(), cornerHistory = loadCornerHistory(), season } = {}) {
  ...
  for (const game of result) {
    ...
    if (football && leagueId && leagueSeason) {
      ...
      if (!standingsCache.has(key)) standingsCache.set(key, Promise.resolve(football(`/standings?league=${leagueId}&season=${leagueSeason}`, 21_600_000)).then(normalizeStandings).catch(() => []));
