...
LIGAS_PRINCIPAIS = {2, 3, 11, 13, 39, 61, 71, 73, 78, 135, 140, 848}
ODD_MINIMA = 1.50
ODD_MAXIMA = 2.30
ODD_MAX_IDADE_SEGUNDOS = 120
...
BOOKMAKER_ID = os.getenv("BOOKMAKER_ID", "8").strip()  # Bet365 na API-Football
...
async def receber_mensagem_redirecionador(event):
    ...
    if contem_palavra(estrategia_nome, palavras_gols) or contem_palavra(nome_do_bot_cabecalho, palavras_gols):
        tipo_mercado = "gol"
    ...
    mensagem_gol = (
        f"⚽ <b>RADAR DE GOLS & MERCADOS PRINCIPAIS</b>\n\n"
        f"⚽ {jogo}\n"
        f"✅ {estrategia_nome.upper() if estrategia_nome else 'ENTRADA SUGERIDA'}\n\n"
        f"📲 <a href='{link}'>Abrir jogo</a>"
    )
    await client.send_message(MEU_CHAT_GOLS, mensagem_gol, parse_mode='html')
    print(f"✅ [Redirecionador] Sinal enviado: {jogo}")
...
def buscar_odds_ao_vivo(fixture_id):
    ...
    if BOOKMAKER_PREFERIDO and BOOKMAKER_ID:
        url += f"&bookmaker={BOOKMAKER_ID}"
    headers = {"x-apisports-key": API_FOOTBALL_KEY}
    try:
        response = requests.get(url, headers=headers, timeout=10)
        if response.status_code == 200:
...
def selecionar_odd_compativel(odds, categoria, entrada, home, away, hg, ag, tempo=None):
    ...
    for mercado in odds or []:
        ...
        if BOOKMAKER_PREFERIDO and _texto_normalizado(bookmaker) != _texto_normalizado(BOOKMAKER_PREFERIDO):
            continue
...
def analisar_oportunidades(tempo, hg, ag, home, away, d, recentes=None, vermelhos=None):
    ...
    if MINUTO_INICIO_GOLS <= tempo <= 85 and gols_atuais <= 3 and abs(hg - ag) <= 1:
        ...
        if tempo <= 15:
            ...
            limites = {"alvo": 8, "chutes": 15, "cantos": 9, "perigosos": 105, "confirmacoes": 5}
        motivos_gols = []
        ...
        pressao_recente = recentes.get("shots_on", 0) >= 2 or recentes.get("total_shots", 0) >= 4
    ...
    # UNDER 2.5 FT = a leitura de que não haverá o terceiro gol. Com dois gols
    # no placar, só é permitido a partir dos 70' para respeitar os 90 minutos.
    placar_compativel_under = gols_atuais <= 1 or (gols_atuais == 2 and tempo >= 70)
    if 25 <= tempo <= 82 and placar_compativel_under:
        limite_alvo = max(2, round(tempo * 0.08))
        limite_chutes = max(6, round(tempo * 0.25))
        limite_perigosos = max(35, round(tempo * 1.05))
        motivos_under = []
        if shots_on_total <= limite_alvo:
            motivos_under.append(f"apenas {int(shots_on_total)} chutes no alvo")
        if total_shots <= limite_chutes:
            motivos_under.append(f"somente {int(total_shots)} finalizações")
        if dangerous_total <= limite_perigosos:
            motivos_under.append(f"baixo volume de ataques perigosos ({int(dangerous_total)})")
        if corners_total <= max(3, round(tempo * 0.07)):
            motivos_under.append(f"poucos escanteios ({int(corners_total)})")
        sem_pressao_recente = recentes.get("shots_on", 0) == 0 and recentes.get("total_shots", 0) <= 1
        xg_baixo = not d["tem_xg"] or d["h_xg"] + d["a_xg"] <= max(0.8, gols_atuais + 0.35)
        if len(motivos_under) >= 3 and sem_pressao_recente and xg_baixo and not (vermelhos["home"] or vermelhos["away"]):
            motivo = random.choice(FRASES_UNDER).format(detalhes=texto_motivo_lista(motivos_under))
            oportunidades.append(("UNDER GOLS", "Menos de 2.5 gols FT", motivo))
    ...
    def domina(lado_shots_on, outro_shots_on, lado_total, outro_total,
        ...
        # Antes dos 10 minutos, além do domínio acumulado, exige atividade no
        # snapshot recente para não decidir por uma amostra isolada.
        recente_valido = tempo >= 10 or recentes.get("shots_on", 0) >= 2 or recentes.get("total_shots", 0) >= 4
...
async def escanear_e_alertar_ao_vivo():
    ...
    for item in jogos:
        try:
            ...
            if league_id not in LIGAS_PRINCIPAIS:
                continue
            ...
            if not candidatas:
                print(f"[Scanner] Nenhum mercado com odd bet365 válida: {home} x {away}")
                continue
