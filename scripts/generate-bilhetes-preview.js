...
const { compactPreviewReport } = require('../lib/bilhetes-generation');
...
const beforeLogicalTotal = app.locals.bilhetesApiUsageDiagnostic.logicalTotal;
const beforeCacheL1 = app.locals.bilhetesApiUsageDiagnostic.cacheHitsL1;
const beforeCacheRedis = app.locals.bilhetesApiUsageDiagnostic.cacheHitsRedis;
...
app.locals.bilhetesFixturesLoader(requested).then(payload => { const games = payload.response || []; return app.locals.bilhetesPreviewGenerator({ games, date: requested, apiUsageDiagnostic: app.locals.bilhetesApiUsageDiagnostic }).then(report => ({ games, report })); }).then(({ games, report }) => {
  ...
  report.apiCalls = { logical: app.locals.bilhetesApiUsageDiagnostic.logicalTotal - beforeLogicalTotal, external: report.apiFootballCalls, cacheHitsL1: app.locals.bilhetesApiUsageDiagnostic.cacheHitsL1 - beforeCacheL1, cacheHitsRedis: app.locals.bilhetesApiUsageDiagnostic.cacheHitsRedis - beforeCacheRedis, byEndpoint: report.externalCallsByEndpoint };
  const output = path.join(__dirname, '..', 'outputs', 'bilhetes-real-preview.json');
  fs.mkdirSync(path.dirname(output), { recursive: true }); fs.writeFileSync(output, JSON.stringify(compactPreviewReport(report), null, 2));
